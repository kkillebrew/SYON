...
function [] = IllusorySizeExp_1000ms_Flicker_Behav(optionsString,subjid,runid)
...
for i=1:length(options.sizeArray)
    for j=1:2   % Load both phases
        ...
        options.circTextureArray{1,i,j} = imread(sprintf('%s%.3f%s%s%s%d%s','./Ball_In_Hallway_Stimuli/ballHallwayTextures/BallHallway_',...
            options.sizeArray(i),'_',options.stimType,'_hallway_7degAlpha_Texture_',j,'.png'));
        
        % Load no hallway textures
        options.circTextureArray{2,i,j} = imread(sprintf('%s%.3f%s%s%s%d%s','./Ball_In_Hallway_Stimuli/ballHallwayTextures/BallHallway_',...
            options.sizeArray(i),'_',options.stimType,'_noHallway_7degAlpha_Texture_',j,'.png'));
        
        % Correct the color values in the textures for monitor being used
        options.circTextureArray{1,i,j} = uint8(options.displayInfo.linearClut(options.circTextureArray{1,i,j}+1).*255);
        options.circTextureArray{2,i,j} = uint8(options.displayInfo.linearClut(options.circTextureArray{2,i,j}+1).*255);

        % Pre-make all neccessary textures for faster drawing
        options.circTextures{1,i,j} = Screen('MakeTexture',options.windowNum,options.circTextureArray{1,i,j});
        options.circTextures{2,i,j} = Screen('MakeTexture',options.windowNum,options.circTextureArray{2,i,j});
    end
...
if options.practice.doPractice == 1
    % Run practice
    [options,data] = IllSizeExp_Flicker_Prac(options,data);
...
%         Screen('FrameRect',options.windowNum,[255 0 0],squeeze(options.textureCoords_Corrected(backgroundIdx,2,:,data.currSizeIdx(n))));
        ...
        % Draw texture
        phaseSwitch = 1;
        Screen('DrawTexture',options.windowNum,options.circTextures{backgroundIdx,data.currSizeIdx(n),phaseSwitch},[],squeeze(options.textureCoords_Corrected(backgroundIdx,1,:,data.currSizeIdx(n))));
                ...
                case 0
                    ...
                    switch flipTest
                        case 1   % Redraw
                            ...
                            % Draw texture
                            Screen('DrawTexture',options.windowNum,options.circTextures{backgroundIdx,data.currSizeIdx(n),phaseSwitch},...
                                [],squeeze(options.textureCoords_Corrected(backgroundIdx,1,:,data.currSizeIdx(n))));
                            %         Screen('FrameRect',options.windowNum,[255 0 0],squeeze(options.textureCoords_Corrected(backgroundIdx,1,:,data.currSizeIdx(n))));
            ...
            switch responseBreak
                case 0   % If no response recorded
                    if buttonsHolder(3) == 1   % Top
                        ...
                        data.rawdata(n,7) = GetSecs-options.stimOnsetTime(n);   % RT
                        responseBreak = 1;
