...
function [] = IllusorySizeExp_200ms_Behav_FixLine(optionsString,subjid,runid)
...
for i=1:length(options.sizeArray)
    ...
    options.circTextureArray{1,i} = imread(sprintf('%s%.3f%s%s%s','./Ball_In_Hallway_Stimuli/ballHallwayTextures/BallHallway_',...
        options.sizeArray(i),'_',options.stimType,'_hallway_7degAlpha_Texture_FixLine_1.png'));
    ...
    options.circTextureArray{2,i} = imread(sprintf('%s%.3f%s%s%s','./Ball_In_Hallway_Stimuli/ballHallwayTextures/BallHallway_',...
        options.sizeArray(i),'_',options.stimType,'_noHallway_7degAlpha_Texture_FixLine_1.png'));
...
if options.practice.doPractice == 1
    % Run practice
    [options,data] = IllSizeExp_FixLine_Prac(options,data);
...
%         Screen('FrameRect',options.windowNum,[255 0 0],squeeze(options.textureCoords_Corrected(backgroundIdx,2,:,data.currSizeIdx(n))));
        ...
        % Draw texture
        Screen('DrawTexture',options.windowNum,options.circTextures{backgroundIdx,data.currSizeIdx(n)},[],squeeze(options.textureCoords));
