...
function [] = IllusorySizeExp_1000ms_Flicker_Behav_FixLine(optionsString,subjid,runid)
...
subjid = [subjid '_1000ms_Flicker_FixLine'];
...
for n=1:length(options.trialOrder)
    ...
    if options.practice.practiceBreak ~= 1
        ...
        Screen('DrawTexture',options.windowNum,options.circTextures_fixOnly{backgroundIdx,data.currSizeIdx(n)},[],squeeze(options.textureCoords));
%         Screen('FrameRect',options.windowNum,[255 0 0],squeeze(options.textureCoords_Corrected(backgroundIdx,2,:,data.currSizeIdx(n))));
        ...
        % Draw texture
        phaseSwitch = 1;
        Screen('DrawTexture',options.windowNum,options.circTextures{backgroundIdx,data.currSizeIdx(n),phaseSwitch},[],squeeze(options.textureCoords));
        ...
        % Wait for response
        text1='Was the top (RIGHT KEY) or bottom (LEFT KEY) sphere bigger?';
        textHeight = RectHeight(Screen('TextBounds',options.windowNum,text1));
        DrawFormattedText(options.windowNum,text1,'center',options.yc-(textHeight/2)-150,options.whiteCol);
...
%                     elseif buttonsHolder(1) == 1   % Bottom
                    elseif keycode(options.buttons.buttonDown)
