...
function [options,data] = IllusorySizeExp_DynamVStat_fMRI()
...
if strcmp('Hallway Present',options.runType)
    options.datafile = [options.datafile '_StatVDyn_HallwayPresent'];
elseif strcmp('Hallway Absent',options.runType)
    options.datafile = [options.datafile '_StatVDyn_HallwayAbsent'];
end
...
options.flipRate = 6;   % Hz of flicker
...
for i=1:length(options.time.blockStart)
    ...
    elseif mod(i,2)==0
        ...
        % Change so that flips only happen on 75% (750ms) of a block (or a second)
        options.time.ballPresent(i,:) = repmat([ones([1 (options.flipRate*2)*.75]) zeros([1 (options.flipRate*2)*.25])],[1 options.time.onTime]);
    end
end
...
if options.escBreak ~= 1
    % Draw the stim
    Screen('DrawTexture',options.windowNum,options.circTextures{hallwayIdx,1,1,fixChangeIdx},[],options.textureCoords);
