...
function [] = IllusorySizeExp_GrayCirc_BothPersp_OLDfixChange_fMRI()
...
options.numCatch = 10;   % Number of catch trials (black box behind fixation)
...
for i=1:length(options.sizeArray)
    for j=1:2   % Load both phases
        for p=1:2   % For no fix change trials and fix change trials
            ...
            options.circTextureArray{1,i,j,p,1} = imread(sprintf('%s%.3f%s%s%s%s%d%s','./BallHallway_',...
                options.sizeArray(i),'_',options.stimType,'_hallway_7degAlpha_Texture_cyan_',fixChange,j,'.png'));
            
            % Load no hallway textures
            options.circTextureArray{2,i,j,p,1} = imread(sprintf('%s%.3f%s%s%s%s%d%s','./BallHallway_',...
            ...
            for h=1:3   % Flip the image to get the second persepctive
                options.circTextureArray{1,i,j,p,2}(:,:,h) = flip(options.circTextureArray{1,i,j,p,1}(:,:,h),2);
                options.circTextureArray{2,i,j,p,2}(:,:,h) = flip(options.circTextureArray{2,i,j,p,1}(:,:,h),2);
            end
            
            % Correct the color values in the textures for monitor being used
            options.circTextureArray{1,i,j,p,1} = uint8(options.displayInfo.linearClut(options.circTextureArray{1,i,j,p,1}+1).*255);
            options.circTextureArray{2,i,j,p,1} = uint8(options.displayInfo.linearClut(options.circTextureArray{2,i,j,p,1}+1).*255);
            options.circTextureArray{1,i,j,p,2} = uint8(options.displayInfo.linearClut(options.circTextureArray{1,i,j,p,2}+1).*255);
            options.circTextureArray{2,i,j,p,2} = uint8(options.displayInfo.linearClut(options.circTextureArray{2,i,j,p,2}+1).*255);
            
            % Pre-make all neccessary textures for faster drawing
            options.circTextures{1,i,j,p,1} = Screen('MakeTexture',options.windowNum,options.circTextureArray{1,i,j,p,1});
            options.circTextures{2,i,j,p,1} = Screen('MakeTexture',options.windowNum,options.circTextureArray{2,i,j,p,1});
            options.circTextures{1,i,j,p,2} = Screen('MakeTexture',options.windowNum,options.circTextureArray{1,i,j,p,2});
            options.circTextures{2,i,j,p,2} = Screen('MakeTexture',options.windowNum,options.circTextureArray{2,i,j,p,2});
        end
    end
    ...
    for p=1:2   % For no fix change trials and fix change trials
        ...
        options.circTextureArray_fixOnly{1,i,p,1} = imread(sprintf('%s%.3f%s%s%s','./BallHallway_',...
            options.sizeArray(i),'_fixOnly_hallway_7degAlpha_Texture_cyan',fixChange,'.png'));
        
        % Load no hallway fix only textures
        options.circTextureArray_fixOnly{2,i,p,1} = imread(sprintf('%s%.3f%s%s%s','./BallHallway_',...
            options.sizeArray(i),'_fixOnly_noHallway_7degAlpha_Texture_cyan',fixChange,'.png'));
        
        % Make the reverse perspective textures
        for h=1:3   % Flip the image to get the second persepctive
            options.circTextureArray_fixOnly{1,i,p,2}(:,:,h) = flip(options.circTextureArray_fixOnly{1,i,p,1}(:,:,h),2);
            options.circTextureArray_fixOnly{2,i,p,2}(:,:,h) = flip(options.circTextureArray_fixOnly{2,i,p,1}(:,:,h),2);
        end
        
        % Correct the color values in the textures for monitor being used
        options.circTextureArray_fixOnly{1,i,p,1} = uint8(options.displayInfo.linearClut(options.circTextureArray_fixOnly{1,i,p,1}+1).*255);
        options.circTextureArray_fixOnly{2,i,p,1} = uint8(options.displayInfo.linearClut(options.circTextureArray_fixOnly{2,i,p,1}+1).*255);
        options.circTextureArray_fixOnly{1,i,p,2} = uint8(options.displayInfo.linearClut(options.circTextureArray_fixOnly{1,i,p,2}+1).*255);
        options.circTextureArray_fixOnly{2,i,p,2} = uint8(options.displayInfo.linearClut(options.circTextureArray_fixOnly{2,i,p,2}+1).*255);
        
        % Pre-make all neccessary textures for faster drawing
        options.circTextures_fixOnly{1,i,p,1} = Screen('MakeTexture',options.windowNum,options.circTextureArray_fixOnly{1,i,p,1});
        options.circTextures_fixOnly{2,i,p,1} = Screen('MakeTexture',options.windowNum,options.circTextureArray_fixOnly{2,i,p,1});
        options.circTextures_fixOnly{1,i,p,2} = Screen('MakeTexture',options.windowNum,options.circTextureArray_fixOnly{1,i,p,2});
        options.circTextures_fixOnly{2,i,p,2} = Screen('MakeTexture',options.windowNum,options.circTextureArray_fixOnly{2,i,p,2});
    end
end
cd ../../../../../fMRI_Task/Stim/
...
options.textureCoords(1,:) = [options.xc-(size(options.circTextureArray{1,i,j,p},1)/2),options.yc-(size(options.circTextureArray{1,i,j,p},2)/2),...
    options.xc+(size(options.circTextureArray{1,i,j,p},1)/2),options.yc+(size(options.circTextureArray{1,i,j,p},2)/2)] +...
    [options.singleCirc.fixationOffset options.singleCirc.fixationOffset];
options.textureCoords(2,:) = [options.xc-(size(options.circTextureArray{1,i,j,p},1)/2),options.yc-(size(options.circTextureArray{1,i,j,p},2)/2),...
    options.xc+(size(options.circTextureArray{1,i,j,p},1)/2),options.yc+(size(options.circTextureArray{1,i,j,p},2)/2)] +...
...
if options.escBreak ~= 1
    % Draw the stim
    Screen('DrawTexture',options.windowNum,options.circTextures{hallwayIdx,1,1,fixChangeIdx,perspIdx},[],options.textureCoords(perspIdx,:));
    switch hallwayIdx
        case 1
            Screen ('FillOval',options.windowNum,options.grayCol,squeeze(options.singleCirc.circCoordTex(perspIdx,1,:))'+[-2 -2 2 2]+...
            ...
            Screen ('FillOval',options.windowNum,options.grayCol,squeeze(options.singleCirc.circCoordTex(perspIdx,2,:))'+[-2 -2 2 2]+...
                [options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2) options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2)]);
        case 2
            Screen ('FillOval',options.windowNum,options.grayCol,squeeze(options.singleCirc.circCoordTex(perspIdx,1,:))'+[-2 -2 2 2]+...
                [options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2) options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2)]);
            Screen ('FrameOval',options.windowNum,options.fixCol,squeeze(options.singleCirc.circCoordTex(perspIdx,1,:))'+[-2 -2 2 2]+...
                [options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2) options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2)]);
            Screen ('FillOval',options.windowNum,options.grayCol,squeeze(options.singleCirc.circCoordTex(perspIdx,2,:))'+[-2 -2 2 2]+...
                [options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2) options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2)]);
            Screen ('FrameOval',options.windowNum,options.fixCol,squeeze(options.singleCirc.circCoordTex(perspIdx,2,:))'+[-2 -2 2 2]+...
                [options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2) options.textureCoords(perspIdx,1) options.textureCoords(perspIdx,2)]);
    end
                    ...
                    case 1
                        ...
                        if options.blockType(blockCount-1) == 1   % 'Balls present'
                            if options.time.ballPresent(blockCount-1,flipCount-1) == 1
                                Screen('DrawTexture',options.windowNum,options.circTextures{hallwayIdx,1,phaseIdx,fixChangeIdx,perspIdx},[],options.textureCoords(perspIdx,:));
