...
function [] = BistableExp_BinoRiv_EEG(options)
...
if ~isfield(options,'doResting')
    options.doResting = 1; % Run 6 45s runs of the resting task to extract IPAF
end
if ~isfield(options,'doZeroHz')
    options.doZeroHz = 1; % Run 2 runs of the 0Hz flicker to extract IPAF
end
if ~isfield(options,'whichIPAFMeasure')
    options.whichIPAFMeasure = 1; % Chose which IPAF measure to use. (1=reting; 2=0 hz)
end
if ~isfield(options,'eegRecording')   % Record EEG
...
if ~isfield(options,'eyeTracking')
    options.eyeTracking = 0;   % Turn on eyetracking
end
if ~isfield(options,'practice')
   options.practice = []; 
end
if ~isfield(options.practice,'doPractice')   % Run practice
...
if ~isfield(options,'screenShot')
    options.screenShot = 0;   % Take screen shot of stimuli and save
end
...
%% Find IPAF
if runid == 1
    % Run resting if flag is set
    if options.doResting == 1
        restingOptions.displayFigs = 1;
        restingOptions.optionsString = optionsString;
        restingOptions.subjid = subjid;
        restingOptions.runid = runid;
        restingOptions.analysisCheck = 0;
        restingOptions.numBlocks = 3;
        restingOptions.eyeOpenClose = [1 2];   % 1 = eyes open; 2 = eyes closed
        restingOptions.restingOnly = 0;
        [restingData restingOptions] = restingTask(restingOptions);
    end
    
    % If no clear peak in resting data then send error and close...
    
    % Run 0hz condition if flag is set
    if options.doZeroHz == 1
        zeroHzOptions.displayFigs = 1;
        zeroHzOptions.optionsString = optionsString;
        zeroHzOptions.subjid = subjid;
        zeroHzOptions.runid = runid;
        zeroHzOptions.analysisCheck = 0;
        [zeroHzData zeroHzOptions] = BistableExp_BinoRiv_0Hz_EEG(zeroHzOptions);
    end
    
%     data.IPAF.rest = restingData.IPAF;
%     data.IPAF.zeroHz = zeroHzData.IPAF;
    options.restingEEGPath = restingOptions.restEEGPath;
    options.zeroHzEEGPath = zeroHzOptions.zeroHzEEGPath;
    
    % % Load in participants IPAF
    % options.flicker.IPAF = zeroHzData.IPAF.alphaPeakAmp_eyesClosed;
    % options.flicker.IPAF = 10.55;
    options.flicker.IPAF = zeroHzData.IPAF.alphaPeakAmp;
    
    % Save participants IPAF to the group file to load for future runs
    if isfile([options.root_path,'\Bistable_Tasks\BinoRiv_Task\EEG_Task\Data\groupIPAF.mat'])
        load([options.root_path,'\Bistable_Tasks\BinoRiv_Task\EEG_Task\Data\groupIPAF.mat'],'groupIPAF');
        
        groupIPAF.subjID{length(groupIPAF)+1} = subjid;
        if options.doResting == 1
            groupIPAF.restingClosed{length(groupIPAF)+1} = restingData.IPAF.alphaPeakAmp_eyesClosed;
            groupIPAF.restingOpen{length(groupIPAF)+1} = restingData.IPAF.alphaPeakAmp_eyesClosed;
        end
        if options.doZeroHz == 1
            groupIPAF.zeroHz{length(groupIPAF)+1} = zeroHzData.IPAF.alphaPeakAmp;
        end
    else
        groupIPAF.restingClosed{1} = restingData.IPAF.alphaPeakAmp_eyesClosed;
        groupIPAF.restingOpen{1} = restingData.IPAF.alphaPeakAmp_eyesOpen;
        groupIPAF.zeroHz{1} = zeroHzData.IPAF.alphaPeakAmp;
        groupIPAF.subjID{1} = subjid;
    end
    
    % Save to file
    save([options.root_path,'\Bistable_Tasks\BinoRiv_Task\EEG_Task\Data\groupIPAF.mat'],'groupIPAF');
    
    clear groupIPAF
else
    % Load in participants IPAF if run is > 1
    load([options.root_path,'\Bistable_Tasks\BinoRiv_Task\EEG_Task\Data\groupIPAF.mat'],'groupIPAF');
    
    options.flicker.IPAF = groupIPAF.zeroHz{strcmp(groupIPAF.subjID,subjid)};
...
% Find flicker rate closest to IPAF to use:
...
for iBlank = 1:n_blank+1
    ...
    n_peaks(iBlank) = 1;
    while length(time_series{iBlank}) < options.numFlips
        time_series{iBlank} = [time_series{iBlank} triangle repmat(0.1,[1 use_blank])];
        n_peaks(iBlank) = n_peaks(iBlank) + 1;
    end
    time_series{iBlank} = time_series{iBlank}(1:options.numFlips);
    
    c2r{iBlank} = complex2real(fft(time_series{iBlank}));
%     figure()
%     plot(c2r{iBlank}.freq*frame_rate,c2r{iBlank}.amp)
    
    time_series_peaks{iBlank} = findpeaks(c2r{iBlank}.amp,'MinPeakHeight',0.01);
    % Find the primary frequency
    time_series_peaks_prim(iBlank) = time_series_peaks{iBlank}(1);
    time_series_peaks_prim_idx(iBlank) = c2r{iBlank}.freq(c2r{iBlank}.amp == time_series_peaks_prim(iBlank))*options.wInfoNew.hz;
end

% Generated list of possible flicker frequencies (From 5-15Hz, which are values used in Cuello):
% [15,13.34,12,10.91,10,9.23,8.58,8,7.5,7.06,6.67,6.32,6,5.72,10.91,5.27,5]
valid_time_series_idx = find(time_series_peaks_prim_idx>=5 & time_series_peaks_prim_idx<=15);
options.flicker.time_series = time_series(valid_time_series_idx);
options.flicker.possibleFlickerVals = time_series_peaks_prim_idx(valid_time_series_idx);

% Find frequencies closest to IPAF:
options.flicker.flickerHz_Actual = [options.flicker.IPAF - 2 options.flicker.IPAF options.flicker.IPAF + 2 3];
for iI=1:length(options.flicker.flickerHz_Actual)
    [~,closestIndex(iI)] = min(abs(options.flicker.flickerHz_Actual(iI) - time_series_peaks_prim_idx'));
    options.flicker.flickerHz(iI) = time_series_peaks_prim_idx(closestIndex(iI));
end

clear time_series_peaks time_series_peaks_max time_series_peaks_max_idx...
    c2r n_peaks triangle use_blank valid_time_series_idx
...
% % For block wave grating flicker stim (can only accept 12, 10, 8 Hz
% % currently):
% % Timing of flicker switches relative to exp start
% for iI = 1:options.numBlocks
%     options.flicker.flickerTimes{iI} = 1/options.wInfoNew.hz:options.flicker.flickerDuration(iI):options.runLength;   % At what times do 'flips' occur
%     options.flicker.flickerFlipVals{iI} = zeros([1 size(options.time.flipTimes,2)])+options.flicker.flickerContrast(1);   % What contrast level will the stim be at for each flip
% end
...
% for iI=1:options.numBlocks
%     for iJ=1:length(options.flicker.flickerTimes{iI})
%         options.flicker.flickerFlipIdx(iJ) = ...
%             find(round(options.time.flipTimes(iI,:),4)==round(options.flicker.flickerTimes{iI}(iJ),4));   % Which # screen flip do we start each flicker flip
%         
%         % Starting at every flicker flip index, increase the contrast for
%         % flickerLength_NumFlips number of flips
%         options.flicker.flickerFlipVals{iI}(options.flicker.flickerFlipIdx(iJ):...
%             options.flicker.flickerFlipIdx(iJ)+options.flicker.flickerLength_NumFlips) = options.flicker.flickerContrast(2);
%     end
% end

% % For sine wave grating flicker stim:
% % Timing of flicker switches relative to exp start
% for iI = 1:options.numBlocks
%     fs = options.wInfoNew.hz;   % Frame rate
%     dt = 1/options.wInfoNew.hz;   % Seconds per sample
%     F = options.flicker.flickerHz(iI);   %   Frequency
%     stopTime = 1/F;   % Total time in once cycle
%     t = (0:dt:stopTime-dt);   % Total number of timepoints
%     flipTimes = sin(2*pi*F*t);
%     
%     % Build 1s worth of individual cycles (120 flips)
%     counter = 0;
%     for iC = length(flipTimes)+1:fs
%         counter=counter+1;
%         if counter > length(t)
%             counter = 1;
%         end
%         flipTimes(iC) = flipTimes(counter);
%     end
%     
%     options.flicker.cycleLength{iI} = length(t);
%     options.flicker.flickerFlipVals{iI} = ...
%         repmat(rescale(flipTimes,options.flicker.flickerContrast(1),options.flicker.flickerContrast(2)),[1 options.runLength]);
%     options.flicker.flickerFlipVals{iI} = round(options.flicker.flickerFlipVals{iI},2);
%     options.flicker.flickerFlipValsUnique{iI} = unique(options.flicker.flickerFlipVals{iI});
%     clear fs dt stopTime t F flipTimes
% end

% For peak flicker stim:
options.flicker.flickerFlipVals = time_series(closestIndex(:));
% Always only 4 contrasts
options.flicker.flickerFlipValsUnique = [.1 0.1+0.5/3 0.1+(2*0.5/3) .6]; 
...
% for iBlock=1:options.numBlocks
fprintf('%s\n','Generating textures...');
for iI=1:length(options.flicker.flickerFlipValsUnique)   % Make different versions of each of the gratings with seperate contrast values (for the 'flicker')
    options.sp.contrast = options.flicker.flickerFlipValsUnique(iI);
...
%         end
        ...
        if options.practice.practiceBreak ~= 1
            ...
            for n=1:options.numFlips
                ...
                contrastHolder = find(options.flicker.flickerFlipVals{m}(n) == options.flicker.flickerFlipValsUnique);
                ...
                if options.flicker.flickerFlipVals{m}(n) == options.flicker.flickerContrast(2) ...
                        && options.flicker.flickerFlipVals{m}(n-1) ~= options.flicker.flickerContrast(2)
    ...
    %% Do last resting block
    options.restingOnly = 1;
    options.numBlocks = 1;
    options.eyeOpenClose = [2];   % Run eyes closed\
    [restingData restingOptions] = restingTask(options);
%     data.resting = restingData;
    clear restingData restingOptions
