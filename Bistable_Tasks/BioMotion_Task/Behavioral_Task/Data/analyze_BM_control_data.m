function [options,dataAve] = analyze_BM_control_data(options,dataAve)
% usage: dataAve = analyze_BM_control_data(options)
% Load in and analyze real switch task data for BM SYON task. 
...
for iSubj = 1:numel(dataAve.A.subjNum)
    ...
    for iJ=1:size(data.control.rawdata,1)   % Block
        ...
        dataAve.A.CV(iSubj,iJ) = ...
                ...
                dataAve.A.perDurAve(iSubj,iJ); % mps 20200519 - kwk adapted for BM 20230206
...
if options.displayFigs
    ...
    if options.includeRelatives == 1
        ...
        set(gca,'XTickLabel',[0:20],'XTick',[1:21])
        clear holderPlot
...
if options.displayFigs
    ...
    title('Real Switch Task (BM)','fontsize',titleFontSize)
    box off
