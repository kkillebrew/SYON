function dataAve = analyze_BM_data(options)
% usage: dataAve = analyze_BM_data(options)
%
% KWK - 20230206
...
%     options.top_dir = '/labs/srslab/data_main/SYON.git/Bistable_Tasks/BioMotion_Task/Behavioral_Task/Data/';
    options.top_dir = 'E:\GitRepos\SYON.git\Bistable_Tasks\BioMotion_Task\Behavioral_Task\Data\';
%     options.top_dir = 'C:\GitRepos\SYON.git\Bistable_Tasks\BioMotion_Task\Behavioral_Task\Data\';
end
if ~isfield(options,'initial_dx_dir')
    options.initial_dx_file_name = 'SYON-InitialDxGroup_DATA_2023-10-31_1357';
%     options.initial_dx_dir = 'C:\GitRepos\SYON.git\Demographics\';
    options.initial_dx_dir = 'E:\GitRepos\SYON.git\Demographics\';
end
...
options.curDur = 'E:\GitRepos\SYON.git\Bistable_Tasks\BioMotion_Task\Behavioral_Task\Data\';

% addpath(genpath('C:\GitRepos\SYON.git\Functions'))
% options.curDur = 'C:\GitRepos\SYON.git\Bistable_Tasks\BioMotion_Task\Behavioral_Task\Data\';

%% pull in data
taskNames = {'A', 'B'};
dataAve.A.dataFileList = dir(fullfile(options.top_dir,'S*BioMotion*.mat'));
dataAve.B.dataFileList = dir(fullfile(options.top_dir,'S*BioMotion*.mat'));
...
options.responseNumCutoff = 0;   % Response # cutoff (out of 7; 64% of trials)
options.responseTimeCutoff = 10;   % Reaction time cutoff (s)
% Switch times:
%     [0,20.323,47.246,64.669,92.492,115.52]
for iSubj = 1:numel(dataAve.A.subjNum)
    ...
    if isfield(tempOptions.options.control,'switchTimesRange')
        dataAve.A.controlSwitchTime{iSubj} = tempOptions.options.control.time.flipTimes(tempOptions.options.control.switchTimesRange(1:end-1)+1);
        dataAve.A.controlSwitchType{iSubj} = tempOptions.options.control.switchIdx(tempOptions.options.control.switchTimesRange(1:end-1)+1);
        ...
        holderCounter2(iSubj) = size(tempOptions.options.control.switchTimesRange,2);
    elseif isfield(tempOptions.options.control,'switchTimeRange')
        dataAve.A.controlSwitchTime{iSubj} = tempOptions.options.control.time.flipTimes(tempOptions.options.control.switchTimeRange(1:end-1)+1);
        dataAve.A.controlSwitchType{iSubj} = tempOptions.options.control.switchIdx(tempOptions.options.control.switchTimeRange(1:end-1)+1);
        dataAve.A.controlSwitchTimeBins{iSubj} = [dataAve.A.controlSwitchTime{iSubj}(:) dataAve.A.controlSwitchTime{iSubj}(:)+options.responseTimeCutoff];
        holderCounter(iSubj) = 0;
        holderCounter2(iSubj) = size(tempOptions.options.control.switchTimeRange,2);
    end
    clear tempOptions
end

%% Exclusions
% Some subjects did not have 7 switches, exclude these subjects
dataAve.A.dataFileList(holderCounter2==11) = [];
dataAve.A.subjID(holderCounter2==11) = [];
dataAve.A.subjNum(holderCounter2==11) = [];
dataAve.A.dateNum(holderCounter2==11) = [];
dataAve.A.controlSwitchTime(holderCounter2==11) = []; 
dataAve.A.controlSwitchTimeBins(holderCounter2==11) = [];
dataAve.A.controlSwitchType(holderCounter2==11) = [];

dataAve.B.dataFileList(holderCounter2==11) = [];
dataAve.B.subjID(holderCounter2==11) = [];
dataAve.B.subjNum(holderCounter2==11) = [];
dataAve.B.dateNum(holderCounter2==11) = [];
...
[options,dataAve] = analyze_BM_control_data(options,dataAve);
...
for iSubj = 1:numel(dataAve.B.subjNum)
    ...
    for iJ=1:size(data.rawdata,1)
        ...
        dataAve.B.CV(iSubj,iJ) = ...
                ...
                dataAve.B.perDurAve(iSubj,iJ); % mps 20200519 - kwk adapted for BM 20230206
...
%     addpath(genpath('/home/shaw-raid/matlab_tools/mpsCode/plotSpread')) alredy in SYON.git functions
    ...
    title('Bi-stable Task (BM)','fontsize',titleFontSize)
    box off
...
if options.displayFigs
    ...
    for iDir = 1:2   % Plot for both directions
        ...
        title(sprintf('%s%s%s','Bi-stable Task (BM - ',directionTitle{iDir},')'),'fontsize',titleFontSize)
        box off
        ylabel('Percept Duration (sec)','fontsize',axisTitleFontSize)
        set(gca,'ylim',[0 100])
        set(gca,'XColor','k','YColor','k')
