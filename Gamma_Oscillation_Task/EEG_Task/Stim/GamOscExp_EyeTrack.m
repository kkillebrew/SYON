...
while ~keycode(options.buttons.buttonEscape)
    ...
    if options.practice.practiceBreak ~= 1
        ...
        if this_b
            ...
            data.blockAcc(options.blockCount) = sum(data.rawdata(data.rawdata(options.blockCountArray(options.blockCount):options.blockCountArray(options.blockCount+1),3)==1,7)>0)/...
                numel(data.rawdata(data.rawdata(options.blockCountArray(options.blockCount):options.blockCountArray(options.blockCount+1),3)==1,7)>0)*100;
            data.blockRT(options.blockCount) = nanmean(data.rawdata(data.rawdata(data.rawdata(options.blockCountArray(options.blockCount):options.blockCountArray(options.blockCount+1),3)==1,7)>0,7));
            
            text5 = sprintf('%s%.1f%s','Your responded on ',data.blockAcc(options.blockCount),'% of trials.');
...
%                 save(sprintf('%s%s%s%s%s%s%s%d','etData_',options.subjID,'_',options.expName,'_',options.datecode,'_',options.blockCount),'etData')
                ...
                cd c:/Users/'EEG Task Computer'/Desktop/SYON.git/Gamma_Oscillation_Task/EEG_Task/Stim
        ...
        if options.eegRecording == 1
            if data.rawdata(n,3) == 1   % 1=speed up trials
                outp(options.addressOut, options.default(n));   % Send trigger
                WaitSecs(0.005);
                default = 0;
                outp(options.addressOut, default);   % Clear the port
            elseif data.rawdata(n,3) == 2   % 3=no speed up trials
        ...
        if options.eyeTracking == 1
            gzDataHolder.data{etData.etCounter} = options.ETOptions.eyeTracker.get_gaze_data();   
        end
