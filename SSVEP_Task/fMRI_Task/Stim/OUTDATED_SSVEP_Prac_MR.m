...
while options.escBreak ~= 1   % loop until escaped
    ...
    if options.escBreak ~= 1 && options.instSwitch == 1
        text1='You will see a flickering checkerboard inside four circles...';
    ...
    if options.escBreak ~= 1 && options.instSwitch == 3
        text1='These will be presetned in 10s blocks.';
        text2='Your task is to keep your eyes fixed';
        text3='on the small white circle in the center of the screen.';
    ...
    if options.escBreak ~= 1 && options.instSwitch == 4
        ...
        while 1
                ...
                switch fixSwitch2
                    case 1
                        switch colorFixSwitch
                            case 1
                                colorFixSwitch = 2;
                                fixCol = options.redCol;   % Fixation color
                            case 2
                                colorFixSwitch = 3;
                                fixCol = options.blueCol;   % Fixation color
                            case 3
                                colorFixSwitch = 4;
                                fixCol = options.greenCol;   % Fixation color
                            case 4
                                colorFixSwitch = 1;
                                fixCol = options.yellowCol;   % Fixation color
                        end
                    case 2
                        fixCol = options.whiteCol;   % Fixation color
                end
            ...
            text1='Sometimes the center white circle will change color.';
            textHeight = RectHeight(Screen('TextBounds',options.windowNum,text1));
            DrawFormattedText(options.windowNum,text1,'center',(textHeight/2)+100,options.fixCol);
    ...
    if options.escBreak ~= 1 && options.instSwitch == 6
        ...
        while 1
            ...
            text1='When it changes to BLUE, press the red (LEFT) button.';
            textHeight = RectHeight(Screen('TextBounds',options.windowNum,text1));
            DrawFormattedText(options.windowNum,text1,'center',(textHeight/2)+100,options.fixCol);
    ...
    if options.escBreak ~= 1 && options.instSwitch == 8
        ...
        while 1
            ...
            text1='At the start and end of the experiment, there will be';
            text2='''BLANK'' blocks, where no checkerboards will appear on the screen.';
            text3='These will also last for 10s each. During these blocks the';
            text4='center circle can still change colors, so keep paying attention.';
            textHeight = RectHeight(Screen('TextBounds',options.windowNum,text1));
            DrawFormattedText(options.windowNum,text1,'center',(textHeight/2)+100,options.fixCol);
            textHeight = RectHeight(Screen('TextBounds',options.windowNum,text2));
            DrawFormattedText(options.windowNum,text2,'center',(textHeight/2)+150,options.fixCol);
    ...
    if options.escBreak ~= 1 && options.instSwitch == 10
        ...
        if options.pracTrialEscBreak == 1 && options.pracFinished ~= 1
            text1='Restart practice trials? Or exit?';
            textHeight = RectHeight(Screen('TextBounds',options.windowNum,text1));
            DrawFormattedText(options.windowNum,text1,'center',(textHeight/2)+100,options.fixCol);
        ...
        if options.pracTrialEscBreak ~= 1
            % Draw the stim
                    ...
                    if ((GetSecs - options.time.expStart) > options.catchTime(catchCounter)) && ...
                        ...
                        switch catchSwitch
                            case 1
                                ...
                                switch options.catchType(catchCounter)
                                    case 1
                                        if keycode(options.buttons.button4)
%                                             disp('r')
                                    ...
                                    case 2
                                        if keycode(options.buttons.button1)
%                                             disp('b')
                                            data.rawdata(catchCounter,3) = 1;
                                            data.rawdata(catchCounter,4) = (GetSecs-options.time.expStart) - options.time.flipTimesActual(blockCount-1,flipCount-1);
                                            catchSwitch = 0;
                                    ...
                                    case 3
                                        if keycode(options.buttons.button4) || keycode(options.buttons.button1)   % Monitor for any buttons press
%                                             disp('g')
                                            data.rawdata(catchCounter,3) = 1;
                                            data.rawdata(catchCounter,4) = (GetSecs-options.time.expStart) - options.time.flipTimesActual(blockCount-1,flipCount-1);
                                            catchSwitch = 0;
                                    ...
                                    case 4
                                        if keycode(options.buttons.button4) || keycode(options.buttons.button1)
%                                             disp('y')
                                            data.rawdata(catchCounter,3) = 1;
                                            data.rawdata(catchCounter,4) = (GetSecs-options.time.expStart) - options.time.flipTimesActual(blockCount-1,flipCount-1);
                                            catchSwitch = 0;
            ...
            % Display accuracy
            text1=sprintf('%s%d%s','Your practice accuracy was: ',nanmean(data.aveAccYG,data.aveAccRB),' %');
...
%     text1='LAST SCREEN!';
    ...
    text4='center white circle and respond RIGHT when the circle turns RED';
