...
function [] = IllContExp_EEG_ScreenShot(optionsString,subjid,runid)
...
options.break_trials = options.numTrials/4+1:options.numTrials/4:options.numTrials;   % Breaks half way through each block
...
data.rawdata = zeros([length(options.varList),10]); 
...
for n=1
    ...
    if options.practice.practiceBreak ~= 1
        ...
        % Force it to be illusory/fragmented for screen shot
        illFragIdx = 2;
        ...
        Screen('FillOval',options.windowNum,options.fixCol,[options.xc-5 options.yc-5 ...
            options.xc+5 options.yc+5]);
        ...
        eval(sprintf('%s%d%s','imwrite(imageArray,''IllContStimFrag',imCounter,'.jpg'')'))
