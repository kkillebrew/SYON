...
function [] = IllContExp_EEG_EyeTrack()
...
%             cd ./EEG_Task/Stim/
    ...
    if options.practice.practiceBreak ~= 1
        ...
        if this_b
            ...
            data.blockAcc(options.blockCount) = nanmean(data.rawdata(options.blockCountArray(options.blockCount):options.blockCountArray(options.blockCount+1),11))*100;
            data.blockRT(options.blockCount) = nanmean(data.rawdata(options.blockCountArray(options.blockCount):options.blockCountArray(options.blockCount+1),10));
        ...
        % MOVING TO AFTER RESPONSE FLIP TO TRY TO FIX TIMING - KWK 20200225
%         % Grab eyetracking data for this trial
%         if options.eyeTracking == 1
%             gzDataHolder{etData.etCounter} = options.ETOptions.eyeTracker.get_gaze_data();
%         end
...
if options.eyeTracking == 1
    ...
    % WANT TO UPDATE TO NEW FOLDER ON SERVER - KWK 20210216
    cd e:/SYON_EyeTracking/
    save(sprintf('%s%s%s%s%s%s%s%d','etData_',options.subjID,'_',options.expName,'_',options.datecode,'_',options.blockCount),'etData','gzData')
