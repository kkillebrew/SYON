...
function [] = IllContExp_Behav()

clear all; close all; sca;
addpath('C:\Users\EEG Task Computer\Desktop\SYON.git\Illusory_Contour_Task\Behavioral_Task\Stim\');
tic;
% switch nargin
%     case 1
%         subjid = [];
...
if exist(fullfile('../../IllContCounterBalance_Behav.mat'),'file')
    load('../../IllContCounterBalance_Behav','IllContCounterBalance');
else
    IllContCounterBalance.subjNum = options.subjID;
    IllContCounterBalance.condOrder = randi(2);
    save('../../IllContCounterBalance_Behav','IllContCounterBalance');
end
...
blockCountArray = [1 options.break_trials(1) length(options.trialOrder)/2+1 options.break_trials(2) options.numTrials];
...
data.xL = [.5:.25:6 6.5:.5:15]; % range of tilt angles to use
data.aL = .5:.5:15; % threshold prior range
...
for n=1:length(options.trialOrder)
        ...
        if options.practice.practiceCheck == 1
            ...
            if options.condOrder == 1   % If illusory first
                ...
                elseif n==length(options.trialOrder)/2+1
                    ...
                    data.blockAcc(blockCount) = nanmean(data.rawdata(blockCountArray(blockCount):blockCountArray(blockCount+1),10))*100;
                    data.blockRT(blockCount) = nanmean(data.rawdata(blockCountArray(blockCount):blockCountArray(blockCount+1),9));
                    ...
                    text3='We''re finished with the ''fat'' and ''thin'' trials!';
    ...
    if options.practice.practiceBreak ~= 1
        ...
        if stairIdx == 4
            options.stim.overallTilt = 15;   % For the 4th staircase, we just want to use max value. (catch trials)
        else
            ...
            switch responseBreak
                case 0
                    ...
                    if buttonsHolder(1) == 1   % Left facing/fat
                        ...
                        % CHANGE HOW WE ARE RECORDING RESP TIME TO
                        % GETSECS-STIM ONSET. ALSO RECORD ALL TIMING ONSETS
                        % IN RAWDATA. DO ACROSS ALL EXPERIMENTS!!! ALSO
                        % MAKE SURE THE TABLE OUTPUT FILE IS CORRECTLY
                        % LABELED.
...
if options.practice.practiceBreak ~= 1
    ...
    if options.analysisCheck == 1
        ...
        data.thresh.max_thresh = 15; % maximum theoretical threshold, exclude if outside % mps 20190730 max = 15
